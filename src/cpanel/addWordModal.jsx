...
import { useReadCookie, useSetCookie } from "../hooks/cookies";
...
const AddWordModal = ({ closeModal }) => {
  ...
  const [newWord, setNewWord] = useState({
    ...
    type: "",
    visable: "false",
    image: readCookie("imgFile") ?? null,
    wordDup: null,
  });

  const [isPhrase, setIsPhrase] = useStateIfMounted(false);

  useEffect(() => {
    //document.body.style.cssText = "overflow:hidden;";

    // Also clear any images that may be set in a cookie
    setCookie("imgFile", "", "-99999");
  }, []);
  ...
  const handleSubmit = async (evt) => {
    ...
    // Trim any white spaces before adding the word

    newWord.name = newWord.name.trim();
        ...
        <div className={css.mobileContainer}>
          ...
          <div className={css.inputContainer}>
            <label className={css.inputField} htmlFor="newWord">
              ...
              />
              {isPhrase && <span className={css.phrase}>Colloquialism</span>}
            </label>
            <label className={css.inputField} htmlFor="newWord">
